...
def request_http(url,headers,payload=None,method='POST'):
    try:
        if payload == None:
            ...
            response = session.request(method, url, headers=headers, data=json.dumps(payload), timeout=30)
...
mint_status = 'AlienOpenedAirBox'
init_excel_header(excel_file_name,[mint_status, 'NextAlienSignTime', 'AlienBoxScore', 'AlienTotalScore'])
...
while need_mint:
    ...
    NextAlienSignTime = excel_data['NextAlienSignTime'].to_list()
    ...
    if dataScope:
        ...
        NextAlienSignTime = NextAlienSignTime[dataScope[0] - 1:configSize]
    ...
    for i, priv in enumerate(PrivateKey):
        ...
        try:
            ...
            #     invitationCodesIndex += 1
            #     invitationCount = 0
            #
            # invitationCode = invitationCodes[invitationCodesIndex]
            # print(f'当前使用邀请码：{invitationCode}，已邀请人数：{invitationCount}')

            print(f'当前使用邀请码：{invitationCode}')

            if not can_sign(NextAlienSignTime[i]):
                print('今日已签到过')
                print('----------------------------------------\n')
                continue
            ...
            invitationCode = current_invite_code
            ...
            if not (isinstance(Minted[i], str) and Minted[i] == 'YES'):
            #     领盒子，拿空投
                print('准备领关怀盒子')
                ...
                data = request_http(url, headers=headers, payload=payload)
                if data['msg'] == 'ok':
                    score = data['data']['blind_box']
                    if score is None:
                        score = 0
                    excel_data.loc[num:num, mint_status] = 'YES'
                    excel_data.loc[num:num, 'AlienBoxScore'] = score
                    print(f'领取成功，获得积分{score}')
            ...
            if data["data"]["checkin"] == 0:
                ...
                if data['msg'] == 'ok':
                    print('签到成功')
                    excel_data.loc[num:num, 'NextAlienSignTime'] = get_next_sign_time()
            else:
                print('今日已签到')
                excel_data.loc[num:num, 'NextAlienSignTime'] = get_next_sign_time()
            ...
            # }
            if data["msg"] == "ok":
                totalScore = data["data"]["total_points"]
                print(f'更新总分：{totalScore}')
                excel_data.loc[num:num, 'AlienTotalScore'] = totalScore
